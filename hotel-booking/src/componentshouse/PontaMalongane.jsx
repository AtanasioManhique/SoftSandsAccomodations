...
const PontaMalongane = () => {
  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        const filtered = res.data.filter((house) => house.location === "Ponta Malongane");
        setHouses(filtered);
      })
  ...
  const visible = houses.slice(currentIndex * itemsPerPage, currentIndex * itemsPerPage + itemsPerPage);
  ...
  return (
    <div className="flex flex-col items-start px-4 md:px-20 pt-8 mt-1 relative">
      ...
      <div className="flex items-center justify-between mb-4 w-full">
        <Link
          to={`/praias/${encodeURIComponent("Ponta Malongane")}`}
